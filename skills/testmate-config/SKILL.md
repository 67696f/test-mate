...
Broad, but not blind. The hook splits the line on `&&`, `||`, `;`, `|` and newlines and checks each
command it would run, so `cd /repo && git push` is caught by its second segment. Within a segment
the match is still substring-based: `git push` blocks `git push --force origin main`.

What it does not block is the pattern appearing purely as data. `echo "git push"` and
`grep -r "npm publish" docs/` are printing and searching, not running — a segment whose command
only reads or prints its arguments is exempt. That exemption is narrow on purpose and drops away
the moment data can become a command again: `echo $(git push)` and ``echo `git push` `` are
blocked, and an interpreter is never inert, so `bash -c 'git push'` is blocked too.

Keep entries specific enough to be worth matching (`npm publish`, not `npm`). If a legitimate
command is still caught, say so and work around it — do not rewrite the command to slip past the
pattern.
